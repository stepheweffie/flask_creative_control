									...
									for(var ind = 0; ind < elems.length; ++ind)	{
										if ( ssns == "0" ) {
											if ( (col == "col10") || (col == "col11") ) {
												elems[ind].innerHTML = "<p><abbr class='tooltip' title='Unlimited if you have a Per Server license; contractually limited if you have a Per Client or Per Limited Server license'>Unlimited or contractually limited</abbr></p>";
											}
										} else {
								...
								if ( updInfo.isValueChanged("MAX_RATE") ) {
									...
									for(var ind = 0; ind < elems.length; ++ind)	{
										...
										if ( !rate || rate == "null") {
											...
											elems[ind].style.color = color;
											elems[ind].innerHTML = "<p><abbr class='tooltip' title='unlimited messages per second for each item of each client'>Unlimited</abbr></p>";
										}
									}
								}
